...
var DEFAULTS = {
  // 下载模式
  manifest: process.env.RAIN_MANIFEST || void 0,
  ...
  // 课程列表模式
  listCourses: false,
  courseDir: process.env.RAIN_COURSE_DIR || void 0,
  lessonDir: process.env.RAIN_LESSON_DIR || void 0,
...
function loadConfig(argv = process.argv) {
  ...
  config.output = import_node_path.default.resolve(config.output);
  if (config.courseDir) {
    config.courseDir = import_node_path.default.resolve(config.courseDir);
  }
...
function validateAndNormalize(manifest) {
  ...
  if (!hasClassroomId && !hasLessons) {
    ...
    manifest.extractedAt = manifest.extractedAt || (/* @__PURE__ */ new Date()).toISOString();
    return manifest;
  }
  if (hasClassroomId && !hasLessons) {
...
function cookieString(cookies) {
  return Object.entries(cookies).map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join("; ");
}
...
async function downloadImage(url, destPath, options = {}) {
  ...
  await (0, import_promises.mkdir)(import_node_path2.default.dirname(destPath), { recursive: true });
...
async function fetchJson(url, options = {}, retry = 3) {
  ...
  for (let attempt = 0; attempt <= retry; attempt++) {
    try {
      ...
      if (attempt < retry) {
        await sleep2(Math.min(1e3 * 2 ** attempt, 1e4));
      }
    }
  }
  throw lastError;
...
// src/extract.js
var REVIEW_API_BASE = "https://changjiang.yuketang.cn/api/v3/classroom-report/student/review";
var DETAIL_API_BASE = "https://changjiang.yuketang.cn/api/v3/classroom-report/student/detail";
var LESSON_SUMMARY_API = "https://changjiang.yuketang.cn/api/v3/lesson-summary/student";
var PRESENTATION_API = "https://changjiang.yuketang.cn/api/v3/lesson-summary/student/presentation";
function buildApiHeaders(manifest, referer) {
...
async function fetchLessonSummary(manifest, lesson, retry = 3) {
...
async function fetchPresentation(manifest, lesson, presentationId, retry = 3) {
  const referer = buildV3Referer(manifest, lesson);
  const headers = buildApiHeaders(manifest, referer);
...
async function extractLessonPresentations(manifest, lesson, retry = 3) {
  let summary;
  try {
...
function extractSlideUrls(timelineList) {
...
async function extractLessonImages(manifest, lesson, retry = 3) {
  return extractLessonPresentations(manifest, lesson, retry);
}

// src/discover.js
var COURSES_API = "https://changjiang.yuketang.cn/v2/api/web/courses/list";
var LESSONS_API_BASE = "https://changjiang.yuketang.cn/v2/api/web/logs/learn";
async function fetchCourseList(manifest, retry = 3) {
...
function findClassroomByName(courses, courseName) {
  if (!courseName) {
...
${available.slice(0, 20).join("\n")}`
  ...
  if (matches.length > 1) {
    const candidates = matches.map((c) => `- ${c.courseName}\uFF08\u73ED\u7EA7\uFF1A${c.className}\uFF0CclassroomId\uFF1A${c.classroomId}\uFF09`).join("\n");
    throw new Error(
...
async function fetchLessonList(manifest, classroomId, retry = 3) {
  ...
  const lessons = data.activities.filter((activity) => activity.type === 14).map((activity) => ({
...
async function discoverCourse(manifest, retry = 3) {
  if (manifest.classroomId) {
    const lessons2 = await fetchLessonList(manifest, manifest.classroomId, retry);
    if (lessons2.length === 0) {
      throw new Error(`classroomId ${manifest.classroomId} \u672A\u627E\u5230\u4EFB\u4F55\u8BFE\u5802\u8BB0\u5F55`);
    }
    manifest.lessons = lessons2.map((l) => ({
      lessonId: l.lessonId,
...
function filterLessons(lessons, filters = {}) {
  ...
  if (filters.latest) {
    result = result.slice().sort((a, b) => String(b.date).localeCompare(String(a.date)));
...
function buildLessonFilters(config) {
  const filters = {};
  if (config.latest) {
    filters.latest = true;
  }
...
function hasActiveLessonFilters(filters) {
...
function getCourseDir(outputRoot, courseName) {
  return import_node_path3.default.join(outputRoot, sanitizeDirName(courseName));
}
function getLessonDir(courseDir, lesson) {
  ...
  return import_node_path3.default.join(courseDir, dirName);
}
function getPresentationDir(lessonDir, lesson, presentation, presentationIndex) {
  ...
  const dirName = safeTitle ? `${padNumber(presentationIndex + 1, 3)}_${safeTitle}` : `${padNumber(presentationIndex + 1, 3)}`;
  return import_node_path3.default.join(lessonDir, dirName);
}
async function ensureCourseMeta(courseDir, manifest) {
  const metaPath = import_node_path3.default.join(courseDir, "meta.json");
  const meta = {
    version: "1.0",
  ...
  await (0, import_promises3.mkdir)(courseDir, { recursive: true });
  await (0, import_promises3.writeFile)(metaPath, JSON.stringify(meta, null, 2), "utf-8");
}
...
async function writeLessonMeta(lessonDir, lesson, downloadResults) {
  const metaPath = import_node_path3.default.join(lessonDir, "meta.json");
  ...
  await (0, import_promises3.mkdir)(lessonDir, { recursive: true });
  await (0, import_promises3.writeFile)(metaPath, JSON.stringify(meta, null, 2), "utf-8");
}
...
// src/report.js
function buildReport(manifest, lessonsResults) {
  ...
  return {
    ...
    failures: failedLessons.flatMap(
      (lesson) => lesson.failedImages.map((failure) => ({
        lessonId: lesson.lessonId,
        date: lesson.date,
        title: lesson.title,
        ...failure
...
function printReport(report, useJson = false) {
  if (useJson) {
...
// src/llm.js
var BASE_URL = "https://token-plan-cn.xiaomimimo.com/v1";
  ...
  for (let attempt = 0; attempt <= retry; attempt++) {
    try {
      ...
      if (isRateLimitError(err)) {
        const delay = Math.min(1e3 * 2 ** attempt, 3e4) + Math.random() * 500;
...
var EXTRACTION_PROMPT = `\u4F60\u662F\u4E00\u540D\u4F18\u79C0\u7684\u8BFE\u7A0B\u7B14\u8BB0\u6574\u7406\u52A9\u624B\u3002\u8BF7\u4ED4\u7EC6\u8BC6\u522B\u8FD9\u5F20\u5E7B\u706F\u7247\u4E2D\u7684\u6240\u6709\u4FE1\u606F\uFF0C\u5E76\u8F93\u51FA\u4E3A\u7ED3\u6784\u5316\u7684 Markdown \u7B14\u8BB0\u3002
...
async function extractFromImage(client, model, imageBase64, mimeType = "image/jpeg", retry = DEFAULT_RETRIES) {
...
var SUMMARY_PROMPT = `\u4F60\u662F\u4E00\u540D\u8BFE\u7A0B\u590D\u4E60\u5927\u7EB2\u751F\u6210\u4E13\u5BB6\u3002\u4E0B\u9762\u662F\u4E00\u7CFB\u5217\u5E7B\u706F\u7247\u7684 Markdown \u7B14\u8BB0\uFF0C\u8BF7\u8DE8\u9875\u9762\u53BB\u91CD\u3001\u6574\u5408\uFF0C\u751F\u6210\u4E00\u4EFD\u5B8C\u6574\u3001\u8BE6\u7EC6\u3001\u4FE1\u606F\u5BC6\u5EA6\u9AD8\u7684 Markdown \u590D\u4E60\u5927\u7EB2\u3002
...
## \u7B2C\u4E8C\u7AE0 xxx
...`;
async function summarizeNotes(client, model, notes, retry = DEFAULT_RETRIES) {
...
var EXTRACTED_DIR = "extracted";
function imagePathToKey(courseDir, imagePath) {
  return import_node_path5.default.relative(courseDir, imagePath).replace(/\\/g, "/");
}
...
async function readState(courseDir) {
  const statePath = import_node_path5.default.join(courseDir, EXTRACTED_DIR, STATE_FILE);
  try {
    const raw = await (0, import_promises4.readFile)(statePath, "utf-8");
    return JSON.parse(raw);
  } catch (err) {
    if (err.code === "ENOENT") {
      return {};
...
async function writeState(courseDir, state) {
  const statePath = import_node_path5.default.join(courseDir, EXTRACTED_DIR, STATE_FILE);
  await (0, import_promises4.mkdir)(import_node_path5.default.dirname(statePath), { recursive: true });
  await (0, import_promises4.writeFile)(statePath, JSON.stringify(state, null, 2), "utf-8");
}
function outputPathForImage(courseDir, imagePath) {
  const relKey = imagePathToKey(courseDir, imagePath);
  const baseName = import_node_path5.default.basename(imagePath, import_node_path5.default.extname(imagePath));
  const dirName = import_node_path5.default.dirname(relKey);
  return import_node_path5.default.join(courseDir, EXTRACTED_DIR, dirName, `${baseName}.md`);
}
...
async function extractNotesFromCourse({
  client,
  courseDir,
  lessonDir,
  ...
  const resolvedCourse = import_node_path5.default.resolve(courseDir);
  const scanDir = lessonDir ? import_node_path5.default.resolve(lessonDir) : resolvedCourse;
  const allImages = await findImageFiles(scanDir);
  let images = allImages;
  if (lessonDir) {
    const resolvedLesson = import_node_path5.default.resolve(lessonDir);
    images = allImages.filter((p) => {
      const rel = import_node_path5.default.relative(resolvedLesson, p);
      return !rel.startsWith("..") && !import_node_path5.default.isAbsolute(rel);
    });
  }
  ...
  const tasks = images.map((imagePath, i) => async () => {
    ...
    if (!force) {
      try {
        await (0, import_promises4.access)(outputPath);
    ...
    try {
      ...
      state[relKey] = { status: "done", outputPath: imagePathToKey(courseDir, outputPath), updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
      results[i] = { imagePath, relKey, outputPath, skipped: false };
    } catch (err) {
      meta.status = "error";
      meta.error = err.message;
      state[relKey] = { status: "error", error: err.message, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
      results[i] = { imagePath, relKey, outputPath, skipped: false, error: err.message };
    }
    await (0, import_promises4.mkdir)(import_node_path5.default.dirname(outputPath), { recursive: true });
    const content = meta.status === "done" ? `${buildFrontmatter(meta)}${markdown}` : `${buildFrontmatter(meta)}`;
    await (0, import_promises4.writeFile)(outputPath, content, "utf-8");
...
var EXTRACTED_DIR2 = "extracted";
var REVIEW_FILE = "review.md";
var DEFAULT_CHUNK_SIZE = 60;
...
async function findExtractedNotes(courseDir, lessonPrefix = "") {
  const notesDir = import_node_path6.default.join(courseDir, EXTRACTED_DIR2);
  const notes = [];
  async function scan(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          const raw = await (0, import_promises5.readFile)(fullPath, "utf-8");
          const { meta, content } = parseFrontmatter(raw);
          const source = meta.source || imagePathToKey2(courseDir, fullPath).replace(/\.md$/, ".jpg");
...
function imagePathToKey2(courseDir, filePath) {
  return import_node_path6.default.relative(courseDir, filePath).replace(/\\/g, "/");
}
async function summarizeCourse({
  ...
  let lessonPrefix = "";
  if (lessonDir) {
    const resolvedLesson = import_node_path6.default.resolve(lessonDir);
    const resolvedCourse = import_node_path6.default.resolve(courseDir);
    if (!resolvedLesson.startsWith(resolvedCourse + import_node_path6.default.sep) && resolvedLesson !== resolvedCourse) {
      throw new Error(`\u8BFE\u65F6\u76EE\u5F55 ${lessonDir} \u4E0D\u5728\u8BFE\u7A0B\u76EE\u5F55 ${courseDir} \u4E0B`);
    }
    lessonPrefix = import_node_path6.default.relative(resolvedCourse, resolvedLesson).replace(/\\/g, "/");
    reviewPath = import_node_path6.default.join(resolvedLesson, REVIEW_FILE);
  } else {
    reviewPath = import_node_path6.default.join(courseDir, REVIEW_FILE);
  }
  if (!force) {
    try {
      await (0, import_promises5.access)(reviewPath);
  ...
  if (notes.length === 0) {
    const location = lessonPrefix ? `${courseDir}/extracted/${lessonPrefix}` : `${courseDir}/extracted/`;
  ...
  await (0, import_promises5.mkdir)(import_node_path6.default.dirname(reviewPath), { recursive: true });
  await (0, import_promises5.writeFile)(reviewPath, markdown, "utf-8");
...
async function runVerifyAuth(config) {
  const manifest = config.manifest ? readManifest(config.manifest) : buildToolManifest(config);
  try {
...
async function runListCourses(config) {
  const manifest = config.manifest ? readManifest(config.manifest) : buildToolManifest(config);
  try {
    const courses = await fetchCourseList(manifest, config.retry);
...
async function runDownload(config) {
  ...
  for (const lesson of manifest.lessons) {
    ...
    if (lesson.needsExtraction) {
      const presentations2 = await extractLessonImages(manifest, lesson, config.retry);
      lesson.presentations = presentations2;
      lesson.needsExtraction = false;
    }
    const presentations = Array.isArray(lesson.presentations) ? lesson.presentations : [];
    ...
    for (const [pptIndex, ppt] of presentations.entries()) {
      ...
      const tasks = (ppt.images || []).map((url, index) => async () => {
        ...
        const destPath = import_node_path7.default.join(pptDir, fileName);
...
async function main() {
  ...
  if (config.summarize) {
    const { hasFailures: hasFailures2 } = await runSummarize(config);
    process.exitCode = hasFailures2 ? 1 : 0;
    return;
  }
